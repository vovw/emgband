...
void emg()
{
    ...
    while (1)
    {
        if (sample_index < FFT_SIZE)
        {
            ...
            float voltage1 = esp_adc_cal_raw_to_voltage(sensor_value1, adc_chars);
            float voltage2 = esp_adc_cal_raw_to_voltage(sensor_value2, adc_chars);
            float voltage3 = esp_adc_cal_raw_to_voltage(sensor_value3, adc_chars);

            // Apply bandpass filter to the voltage signals
            float signal1 = bandpass_filter_75_150(voltage1);
            float signal2 = bandpass_filter_75_150(voltage2);
            float signal3 = bandpass_filter_75_150(voltage3);
